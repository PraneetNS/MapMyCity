    ...
    try:
        ...
        if os.path.exists(migrations_dir):
            ...
            for filename in migration_files:
                ...
                print(f"FastAPI Backend: Applying migration {filename}...")
                with open(file_path, "r", encoding="utf-8") as f:
                    sql_content = f.read()
                
                async with async_session() as db:
                    try:
                        # Executing commands in migration file
                        # We execute the script inside a transaction
                        await db.execute(text(sql_content))
                        await db.commit()
                        print(f"FastAPI Backend: Successfully applied migration {filename}")
                    except Exception as migration_err:
                        await db.rollback()
                        print(f"FastAPI Backend: Warning/Error running migration {filename}: {migration_err}")
        else:
...
# Helper for JSON serialization of datetimes and UUIDs
...
@app.post("/submissions/passive-batch", response_model=PassiveBatchResponse, status_code=201, tags=["Submissions"])
async def create_passive_batch(
    data: PassiveBatchSubmit,
    background_tasks: BackgroundTasks,
    db: AsyncSession = Depends(get_db)
):
    """
    Accepts a batch of passive accelerometer road quality jolts.
    Inserts each jolt as a submission record and runs clustering.
    """
    if USE_MOCK:
        for jolt in data.jolts:
            row = {
                ...
                "captured_at": jolt.captured_at,
                "submitted_at": datetime.now(timezone.utc),
                "status": "approved",
                "notes": f"Accelerometer spike intensity: {jolt.intensity:.2f}G",
                "p_hash": None,
                "flags": [],
                "cluster_id": None
            }
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to batch insert passive submissions: {str(e)}")
...
async def get_submissions(
    ...
    query = text("""
        ...
        WHERE (:status IS NULL OR s.status = :status)
          AND (:mission_type IS NULL OR s.mission_type = :mission_type)
          AND (
            (:min_lat IS NULL OR :min_lon IS NULL OR :max_lat IS NULL OR :max_lon IS NULL)
            OR ST_Within(s.location::geometry, ST_MakeEnvelope(:min_lon, :min_lat, :max_lon, :max_lat, 4326))
          )
